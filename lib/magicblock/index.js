...
function MagicBlockEngine( _opts ) {
  ...
  function parseAttrs (attrRaw) {
    // FIXME: if( !opts.attrStrAllowClass && !opts.attrStrAllowColor ) return {}; // or false?
    let classAttr = []
    let colorAttr = []
    attrRaw.replace(/([\.#])([\w\-]+)/g, function (match, $1, $2) {
      if (opts.attrStrAllowClass && $1 == '.') {
        classAttr.push($2)
      }else if ( opts.attrStrAllowColor && $1 == '#') {
        if ($2.match(/^[\da-fA-F]+$/)) {
          colorAttr.push($1+$2)
        } else {
          colorAttr.push($2)
  ...
  function magic (data) {
    ...
    if (matched = data.match(/^<a[^>]+href="(.*?)".*?>(.*?)<\/a>/)) {
      ...
      // Begin hackIframely
      if( opts.hackIframely ){
        data = data.replace(/<<hackIframely>>/g,'');
      }
      // End hackIframely
